...
```
图形用户界面（Graphical User Interface，简称 GUI），比如：

Navicat，DataGrip，SQLyog，PL/SQL等
...
-- 查询英语成绩为null
	SELECT * FROM student WHERE english = NULL; -- 错误示例。null值不能使用 = （!=） 判断
...
-- 查询姓名是3个字的人
	...
	还可以用函数
	SELECT * FROM student WHERE length(NAME) = 3; -- Oracle
	SELECT * FROM student WHERE char_length(NAME) = 3; -- MySQL


-- 查询姓名中包含德的人
	SELECT * FROM student WHERE NAME LIKE '%德%';
	还可以用函数
	SELECT * FROM student WHERE instr(NAME, '德') > 0; -- Oracle
	SELECT * FROM student WHERE locate('德', NAME) > 0; -- MySQL
...
Connection_—>socket网络流

Statement/PreparedStatement_—>输出流

## 3.快速入门

SSM版步骤如下所示，Spring Boot的话引入maven依赖就可以
...
MySQL对于事务默认是不支持的，只是有某些存储引擎中如：innodb可以支持；而Oracle对于事务是完全支持的。
...
| 10   | in()                     | in内最多1000个数据，多于1000用in(1,2,3,...,1000) or in(1001) | in内的数据数量没有限制，可对sql的长度有限制，默认4M          |                                                              |
...
| 12   |              | length（str）                                           | char_length(str)                                             | 函数名称不同：<br/> Oracle: SELECT length('AAAASDF') VALUE FROM DUAL<br/> MySQL: SELECT char_length('AAAASDF') VALUE |
