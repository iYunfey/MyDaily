...
public class StringRegexUtils {
    ...
    public static boolean isMobileNumber(String string) {
        if (string.matches("^[1][3,4,5,8][0-9]{9}$")) {
            return true;
        }
        return false;
    }
...
//r的使用  
System.out.printf("HH:MM:SS PM格式（12时制）：%tr%n",date);//HH:MM:SS格式（24时制）：10:43:36
//t的使用  
System.out.printf("HH:MM:SS格式（24时制）：%tT%n",date);//HH:MM:SS PM格式（12时制）：10:43:36 上午
//R的使用  
System.out.printf("HH:MM格式（24时制）：%tR",date);//HH:MM格式（24时制）：10:43
// LocalDateTime也适用
LocalDateTime localDateTime = LocalDateTime.now();
System.out.println(String.format("年-月-日格式：%tF%n",localDateTime));//年-月-日格式：2021-12-01
